...
# ──────────────────────────────────────────────────────────────────────────────
# HELPER: оффер пробный периода 3 дня за 1 ₽ с прямой кнопкой оплаты (без чекбокса)
# ──────────────────────────────────────────────────────────────────────────────
def build_trial_offer(user_id: int) -> tuple[str, InlineKeyboardMarkup]:
    ...
    как первый тариф 1m с пробный периодом. Ссылка — результат youmoney.create_pay_ex.
    """
    plan = TARIFFS["1m"]
...
def _create_links_for_selection(user_id: int) -> tuple[Optional[str], Optional[str]]:
    ...
    ВАЖНО: для пробный периода «3 дня за 1 ₽» разрешаем ТОЛЬКО рекуррентные платежи (с сохранением метода).
...
async def process_yookassa_webhook(bot: Bot, payload: Dict) -> Tuple[int, str]:
    try:
        ...
        # Убедимся, что пользователь есть в app DB (для пробный периода/истории)
