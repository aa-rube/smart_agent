...
Важная информация: генератор планировок пока работает в тестовом режиме, и в отдельных случаях возможны ошибки (особенно при обработке многокомнатных помещений) и мы честно предупреждаем вас об этом ❤️

Чтобы результат был максимально точным:
 • Используйте четкие планы без размытий.
 • Убирайте лишние элементы (надписи, мебель, водяные знаки).
 • Линии стен, дверей и окон должны быть прямыми и не пересекаться.
