...
1️⃣ Загрузи план/чертёж помещения — подойдет изображение (jpg/png) или PDF (1 страница). Ссылка на картинку тоже ок.
...
*Важная информация о Генераторе планировок (тестовая версия)*
Сервис работает в тестовом режиме, и в отдельных случаях возможны ошибки (особенно при обработке многокомнатных помещений) и мы честно предупреждаем вас об этом.

✅ Чтобы результат получился максимально удачным, соблюдайте простые требования к загружаемой планировке:
Загружайте файл в *хорошем качестве* (чёткое изображение без размытий).
План должен быть с *прямыми углами и читаемыми размерами.*
Используйте изображения без лишних элементов (фото мебели, водяные знаки, надписи и цифры мешают распознаванию).
Все линии перегородок, окон и дверных проемов должны быть *четкими, не пересекаться и не накладываться друг на друга.*
Чем качественнее исходная планировка, тем точнее и красивее будет итоговый результат ❤️
