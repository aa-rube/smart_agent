...
# Системный промпт (как и был): методология Х–П–В + требования к тексту
...
# ------------------ Карты лейблов для select-полей (используются и в UI, и в фабрике) ------------------
# NB: Эти словари уже используются в хендлерах для кнопок/клавиатур. Здесь — единый источник правды.
# Примерные значения — вы можете адаптировать под свои варианты.
DESCRIPTION_TYPES = {
    "flat":   "Квартира",
    "house":  "Дом",
    "office": "Офис",
...
# ------------------ Единый шаблон пользовательского сообщения для описания ------------------
# Все плейсхолдеры используются фабрикой compose_description_user_message(...)
# Значения приходят уже в «безопасном» виде.
DESCRIPTION_USER_TEMPLATE_RU = '''
Сгенерируй продающее, информативное описание объекта недвижимости для объявления и презентации.
Соблюдай гайд Х–П–В, пиши фактами, избегай «воды», и заверши явным CTA (приглашение к просмотру/звонку).

— Тип объекта: {type_label}
— Класс (если квартира): {apt_class_label}
...
— Год постройки / Состояние: {year_state}
— Коммуникации (отопление/вода/газ/электричество/интернет): {utilities}
— Особенности / удобства (балкон/парковка/лифт/охрана и т. п.): {amenities}
— Комментарий агента: {comment}

Сформируй структурированный, читабельный текст с выгодами для покупателя, упоминая ключевые факты из анкеты.
'''
