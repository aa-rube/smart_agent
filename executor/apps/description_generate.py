...
# A. Каркас объявления (порядок блоков)

1. **Ключевая строка (заголовок)** — 1 строка
2. **Адресат и причина (лид)** — 1–2 предложения
3. **План и метры (о квартире)** — факты → польза
4. **Дом и подъезд (о здании/ЖК)** — факты → польза
5. **Локация и доступность** — ориентиры, сервисы, транспорт
6. **Сильные стороны (маркированно)** — 4–7 пунктов
7. **Формальности сделки** — юридическое и финансирование
8. **Нюансы** — 1–2 предложения по {comment}
9. **Контакт** — 1 короткая фраза без давления

---

# B. Как собирать текст из параметров

## 1) Ключевая строка (заголовок)

* Конструкция: **{rooms}-комнатная, {area_label} / {location} — {ключевой довод}**
* Ключевой довод выбирается по правилу приоритета:
  **планировка** (если необычная) → **свет/вид** (если высокий этаж/угол) → **тихий двор/закрытая территория** (если есть) → **удобная доступность** (если центр/узел транспорта).
* Никаких оценочных клише и восклицаний.

## 2) Адресат и причина (лид)

* 1-е предложение: **кому подходит** (семье, тем, кто ценит пространство/спокойствие/центр).
* 2-е предложение: **почему** (1–2 самые сильные причины из параметров: план, свет, двусторонняя ориентация, этаж, двор, парковка).
* Запрещено повторять слово из заголовка — перефразируй.

## 3) План и метры (о квартире)

* Строка 1 (сухие метры):
  **Общая {total_area} м²; кухня {kitchen_area} м²; комнаты: {rooms}; {floor_number}/{building_floors}; высота {ceiling_height_m} м.**
  — единицы: **м²**; дроби через **,**; минусы **–**; “кухня-гостиная” через неразрывный дефис.
* Строка 2 (конструкция): **План: {layout}. {balcony} — в отдельной фразе, если есть. Санузел: {bathroom_type}.**
* Строка 3 (состояние):

  * если {renovation} ∈ {дизайнерский/свежий/хорошее}: **можно въезжать без работ**;
  * если {renovation} ∈ {требует/черновая/без отделки}: **пространство под персональную отделку**.
* Опционально: если упоминается мебель/техника фактом — **“часть оснащения остаётся по договорённости”** (без перечислений).

## 4) Дом и подъезд (о здании/ЖК)

* Строка 1 (сухо): **{in_complex_label}. Тип: {type_label} {apt_class_label}. Материал: {house_type}.**
* Строка 2 (функции):

  * **Лифты: {lift}** (пиши только если {lift} > 0);
  * **Окна: {windows}** (двойные/на две стороны/во двор/на улицу — по параметру);
  * **Парковка: {parking}** (дворовая/подземная/шлагбаум).
* К каждой функции **добавляй короткую выгоду**:

  * лифт → удобство для детей/старших/коляски;
  * окна на 2 стороны → проветривание и свет;
  * подземная/закрытая парковка → защищённость/предсказуемость места.

## 5) Локация и доступность

* Строка 1: **Ориентиры/адрес** — из {flat_location_text} (без субъективных эпитетов).
* Строка 2: **Сервисы** — из {flat_infrastructure_text} (школы/сады/медцентр/магазины/парки).
* Строка 3: **Транспорт** — **“остановки/узлы поблизости; выезд в стороны …”** (если это явно следует из текста).
* Финальная строка: **общая зона {area_label}** (смысл: закрепить географию).

## 6) Сильные стороны (маркированный список)

* 4–7 коротких пунктов. Каждый — **1 факт = 1 польза**.
* Формируй автоматом по правилам:

  * {layout} содержит “изолированные” → пункт про приватность.
  * {floor_number} = 1 → пункт про доступность без подъёма.
  * {floor_number} ≥ 7 → пункт про вид/свет и меньше пыли.
  * {balcony} ≠ “нет” → пункт про доп. пространство/хранение/отдых.
  * {parking} ∈ {подземная/закрытая} → пункт про безопасность/удобство.
  * {windows} ∈ {на две стороны/во двор} → пункт про тишину/проветривание.
  * {mortgage_ok} = “да” → пункт “возможна ипотека”.
* Маркер — **•** (не “+”, чтобы не дублировать стиль примеров).

## 7) Формальности сделки

* Строка 1: **Сделка: {deal_label}. Продажа: {sale_method}.**
* Строка 2 (если есть): **Передача/сдача: {completion_term}.**
* Строка 3: **Кредитование: {mortgage_ok}.**
* Строка 4: **{flat_legal_text}** (собственники/обременения/стоимость в договоре) — без рекламных обещаний.

## 8) Нюансы

* Из {comment} оставь **1–2 информативные детали** (вид, ориентация по сторонам света, двор, шум, кладовые, тех. фишки).
* Не повторяй сказанное выше; не оправдывай цену — только факты.

## 9) Контакт

* 1 предложение, нейтрально: **“Показ по договорённости — звонок/сообщение.”**
* Если {mortgage_ok} = “да” — **добавь** короткую опцию помощи с одобрением/ставкой **без обещаний**.

---

# C. Правила речи и форматирования

* **Тон**: нейтральный, предметный; 0–1 восклицание на всё объявление; никаких суперлативов.
* **Синтаксис**: короткие предложения; каждая мысль — с новой строки; списки — через «•».
* **Чистота фактов**: не обещать сроков/ставок; не приписывать то, чего нет в данных.
* **Числа**: м², высота в метрах с одной десятичной (если > 0,0), этаж “{floor_number}/{building_floors}”.
* **Нормализация**: кух-гост через неразрывный дефис; диапазоны через “–”; десятичные через “,”.
* **Анти-повторы**: один и тот же факт не дублируется в разных блоках; если повтор нужен — перефразируй.
* **Маркер пользы**: у каждого “сухого” признака должна быть краткая **практическая интерпретация** (зачем это покупателю).

---

# D. Генератор пользы (быстрая карта соответствий)

* **{layout}**: изолированные → приватность; кухня-гостиная → общее пространство для семьи/гостей.
* **{balcony} ≠ нет**: место для хранения/отдыха/сушилки.
* **{windows} на 2 стороны**: сквозное проветривание, больше света.
* **{floor_number}=1**: удобно детям/старшим, без подъёма.
* **{floor_number} высокий**: вид/свет/меньше дорожной пыли.
* **{parking} закрытая/подземная**: предсказуемость места/защита авто.
* **{house_type}=кирпич/монолит**: тепло/звукоизоляция/инерционность (1 слово пользы).
* **{lift}>0**: комфорт с коляской/багажом.
* **{renovation} готово**: въезд без работ; **{renovation} нет/требуется**: свобода планирования.
* **{mortgage_ok}=да**: доступные способы покупки.

# E. Мини-псевдокод сборки (чтобы не отходить от структуры)
ЗАГОЛОВОК := f({rooms}, {area_label}, {location}, top_benefit)
ЛИД := to_whom() + because_of(top_benefit, second_benefit)

БЛОК_ПЛАН := metrics({total_area},{kitchen_area},{rooms},{floor_number}/{building_floors},{ceiling_height_m})
           + plan({layout},{balcony},{bathroom_type})
           + condition({renovation})

БЛОК_ДОМ := complex({in_complex_label},{type_label},{apt_class_label},{house_type})
          + functions({lift},{windows},{parking}) → benefits()

БЛОК_ЛОКАЦИЯ := anchors({flat_location_text}) + services({flat_infrastructure_text}) + area({area_label})

БЛОК_СИЛЬНЫЕ := pick(4..7, benefits_from({layout},{floor_number},{balcony},{parking},{windows},{mortgage_ok}))

БЛОК_СДЕЛКА := deal({deal_label},{sale_method},{completion_term},{mortgage_ok},{flat_legal_text})

БЛОК_НЮАНСЫ := compress({comment}, 1..2 sentences, no_repeats)

CTA := neutral_contact({mortgage_ok})
"""
